akx_cell_t *is_int_impl(akx_runtime_ctx_t *rt, akx_cell_t *args) {
  if (akx_rt_list_length(args) != 1) {
    akx_rt_error(rt, "?int requires exactly 1 argument");
    return NULL;
  }
  ...
  int result = 0;
  if (evaled) {
    result = (akx_rt_cell_get_type(evaled) == AKX_TYPE_INTEGER_LITERAL);
  }
