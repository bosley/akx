...
akx_cell_t *is_dir_impl(akx_runtime_ctx_t *rt, akx_cell_t *args) {
  if (!args) {
    akx_rt_error(rt, "fs/?is-dir: requires a path");
    return NULL;
  }

  akx_cell_t *path_cell = akx_rt_eval(rt, args);
  if (!path_cell || !akx_rt_cell_is_type(path_cell, AKX_TYPE_STRING_LITERAL)) {
    akx_rt_error(rt, "fs/?is-dir: path must be a string");
    if (path_cell)
  ...
  int is_dir = (result == 0 && S_ISDIR(st.st_mode));
  ...
  akx_rt_set_int(rt, result_cell, is_dir ? 1 : 0);
  return result_cell;
}
