akx_cell_t *eqtest(akx_runtime_ctx_t *rt, akx_cell_t *args) {
  ...
  if (akx_rt_cell_get_type(left) == akx_rt_cell_get_type(right)) {
    if (akx_rt_cell_get_type(left) == AKX_TYPE_INTEGER_LITERAL) {
      equal = (akx_rt_cell_as_int(left) == akx_rt_cell_as_int(right));
    } else if (akx_rt_cell_get_type(left) == AKX_TYPE_REAL_LITERAL) {
      equal = (akx_rt_cell_as_real(left) == akx_rt_cell_as_real(right));
    } else if (akx_rt_cell_get_type(left) == AKX_TYPE_STRING_LITERAL) {
      const char *ls = akx_rt_cell_as_string(left);
      const char *rs = akx_rt_cell_as_string(right);
      equal = (strcmp(ls, rs) == 0);
    } else if (akx_rt_cell_get_type(left) == AKX_TYPE_SYMBOL) {
